import { Message, Client, VoiceConnection, StreamDispatcher } from "discord.js";
...
function checkVoiceChannel(message: Message) {
    var server = servers[message.guild.id];
...
export async function messageManage(client: Client, message: Message, prefix: string) {
    ...
    if (message.content.startsWith(`${prefix}`)) {
        ...
        switch (args[0]) {
            ...
            case 'stop':
                var server: any = servers[message.guild.id];
                console.log("Splice");
                if (message.guild.voiceConnection) {
            ...
            case 'volumen':
                ...
                var server = servers[message.guild.id];
                let volumen: number = Number(args[1]);
